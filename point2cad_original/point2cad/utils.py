...
# geomdl import removed: `tessalate_points` (the only consumer) is unused on
# the run path. Keeping the import would force a geomdl install for no reason.
...
def normalize_points(points, anisotropic=False):
    ...
    if anisotropic:
        scale = std.reshape((1, 3)) + EPS
        points = points / scale
    else:
        scale = float(np.max(std) + EPS)
        points = points / scale
    return points.astype(np.float32), mean, R, scale
...
def tessalate_points(points, size_u, size_v, mask=None):
    # Disabled: depends on geomdl.tessellate.make_triangle_mesh which has been
    # removed (see import comment above). The fast path `tessalate_points_fast`
    # in fitting_utils.py is what actually runs.
    raise NotImplementedError(
        "tessalate_points is disabled (geomdl removed); use tessalate_points_fast"
    )
