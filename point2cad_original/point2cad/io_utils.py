...
def _normalize_type(t):
    """Match point2cad_repr's surface vocabulary."""
    return "inr" if t == "open_spline" else t


def _write_types_sidecar(meshes, out_path):
    types = [_normalize_type(m["info"]["type"]) for m in meshes]
    with open(os.path.join(os.path.dirname(out_path), "types.json"), "w") as f:
        json.dump(types, f)
...
def save_clipped_meshes(pm_meshes, out_meshes, color_list, out_path):
    ...
    for p in range(len(out_meshes)):
        ...
        clipped_mesh.export(os.path.join(os.path.dirname(out_path), f"cluster_{p}.ply"))

    clipped = trimesh.util.concatenate(clipped_meshes)
    clipped.export(out_path)
    _write_types_sidecar(out_meshes, out_path)
